...
I’ll map EVSYS-capable ports from the ATmega4809 header, then translate those to Nano Every Arduino pin numbers from the active variant so you get an exact usable pin list.

No matches found. Your search pattern might be excluded completely by either the search.exclude settings or .*ignore files.
If you believe that it should have results, you can check into the .*ignore files and the exclude setting (here are some excluded patterns for reference:[**/node_modules,**/bower_components,**/*.code-search]).
Then if you want to include those files you can call the tool again by setting "includeIgnoredFiles" to true.

Read [](file:///c%3A/Users/rpb/.platformio/packages/framework-arduino-megaavr/variants/nona4809/pins_arduino.h)

Read [](file:///c%3A/Users/rpb/.platformio/packages/framework-arduino-megaavr/variants/nona4809/pins_arduino.h)

On the ATmega4809, GPIO event generators are limited to PORT0 and PORT1 pin sources, which correspond to Port A and Port B pins only.

For Arduino Nano Every (your active nona4809 variant), that means EVSYS-compatible Arduino pin numbers are the ones mapped to PAx or PBx:
...
- D9 → PB0
- D10 → PB1
- D24 → PB5 (not on standard Nano header; used by debug USART mapping)
- D25 → PB4 (not on standard Nano header; used by debug USART mapping)

Not EVSYS GPIO generators on this board mapping: D0, D1, D3, D4, D6, D8, D11–D21 (these are on ports C/D/E/F in this variant).

So your D4 input is not EVSYS-pin-generator compatible in this core mapping.
